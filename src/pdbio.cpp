...
void get_mmcif_coordnum(FILE* in, int* natoms)
{
    ...
    char line[MMCIF_STRLEN];  //store line
    
    *natoms = 0;
    while (fgets2(line, MMCIF_STRLEN, in))
    {
        std::string line_string(line);  //convert char array to string
        if (line_string.find("_atom_site.group_PDB") != std::string::npos)  //find the unique label
        {
            while (line[0] != '#') 
            {
                if (line[0] != '_'){ 
		        (*natoms)++;  // accumulate atom number
                }
...
read_atom_mmcif(t_symtab* symtab, const char line[], int natom, t_atoms* atoms, rvec x[], int *chainnum, int pdbtype_n, int anr_n, int anm_n, int resnm_n, int rnr_n, int elem_n, int xc_n, int yc_n, int zc_n, int occup_n, int bfac_n, int chainid_n)
{
    ...
    t_atom*       atomn;
    char          pdbtype[MAX_SIZE], anr[MAX_SIZE], anm[MAX_SIZE], anm_copy[MAX_SIZE], resnm[MAX_SIZE], rnr[MAX_SIZE], elem[MAX_SIZE];  //store data as char array
    char          xc[MAX_SIZE], yc[MAX_SIZE], zc[MAX_SIZE], occup[MAX_SIZE], bfac[MAX_SIZE];  //store data as char array
    unsigned char resic;
    char          altloc, chainid;
    int           resnr, atomnumber;
    ...
    if (natom >= atoms->nr)
    {
        gmx_fatal(FARGS, "\nFound more atoms (%d) in cif file than expected (%d)", natom + 1, atoms->nr);
    }

    char temp[MAX_SIZE];  //temporary char array
    int tempIndex = 0;
    int j = 0;

    for (int i = 0; line[i] != '\0'; i++){
        if (line[i] != ' '){  //divide line by whitespace
            temp[tempIndex] = line[i];
            tempIndex++;
        } 
        else if (tempIndex != 0) 
        {
            temp[tempIndex] = '\0';  //add '\0' to end 
            tempIndex = 0;
            j++;
            if (j == pdbtype_n){
            	strcpy(pdbtype, temp);
                ...
                strcpy(anm_copy, anm);
                trim(anm);
                anm_copy[3]=anm_copy[2];
                anm_copy[2]=anm_copy[1];
                anm_copy[1]=anm_copy[0];
                anm_copy[0]=' ';
                rtrim(anm_copy);
            }
            else if (j == resnm_n){
                ...
                trim(resnm);
            	    ...
            	    chainid = temp[0];
            	    if (temp[1] != '\0'){
            	        *chainnum = (chainid - 'A' + 1) + (temp[1] - 'A' + 1) * 26;
            	    }
                    else{
                        *chainnum = chainid - 'A' + 1;
                    }
        	}
        	else if (j == rnr_n){   
            	    strcpy(rnr, temp);
    ...
    //code below are copied from read_atom
    if (atoms->atom)
    {
        atomn = &(atoms->atom[natom]);
        if ((natom == 0) || atoms->resinfo[atoms->atom[natom - 1].resind].nr != resnr
    ...
    if (atoms->pdbinfo)
    {
    	if (strcmp(pdbtype,"ATOM") == 0){
    	    atoms->pdbinfo[natom].type   = PdbRecordType::Atom;
    	}
    	else if (strcmp(pdbtype,"HETATM") == 0){
    	    atoms->pdbinfo[natom].type   = PdbRecordType::Hetatm;
    	}
        atoms->pdbinfo[natom].atomnr = strtol(anr, nullptr, 10);
        atoms->pdbinfo[natom].altloc = altloc;
        strcpy(atoms->pdbinfo[natom].atomnm, anm_copy);
                 ...
                 gmx_conect conect)
{
    ...
    int           pdbtype_n = 1, anr_n = -1, anm_n = -1, resnm_n = -1, rnr_n = -1, elem_n = -1; // mark label's location
    int           xc_n = -1, yc_n = -1, zc_n = -1, occup_n = -1, bfac_n = -1, chainid_n = -1; // mark label's location
    int           i = 0;
    
    while (fgets2(line, MMCIF_STRLEN, in) != nullptr) 
    {
        std::string line_string(line);  // convert char array to string
        if (line_string.find("_atom_site.group_PDB") != std::string::npos)  // find unique label
        {
            while (line[0] != '#')  // data box begins and ends with '#'
            {
                if (line[0] == '_'){  // label begins with '_'
                    i++;
                    std::string line_string(line);
                    if (line_string.find("_atom_site.id") != std::string::npos){
                        anr_n = i;  //store the location
                    }
                    else if (line_string.find("_atom_site.type_symbol") != std::string::npos){
                        elem_n = i;
                    }
                    else if (line_string.find("_atom_site.label_atom_id") != std::string::npos){   
                        anm_n = i;
                    }
                    else if (line_string.find("_atom_site.label_comp_id") != std::string::npos){
                        resnm_n = i;
                    }
                    else if (line_string.find("_atom_site.label_asym_id") != std::string::npos){   
                        chainid_n = i;
                    }
                    else if (line_string.find("_atom_site.auth_seq_id") != std::string::npos){   
                        rnr_n = i;
                    }
                    else if (line_string.find("_atom_site.Cartn_x") != std::string::npos){
                        xc_n = i;
                    }
                    else if (line_string.find("_atom_site.Cartn_y") != std::string::npos){
                        yc_n = i;
                    }
                    else if (line_string.find("_atom_site.Cartn_z") != std::string::npos){
                        zc_n = i;
                    }
                    else if (line_string.find("_atom_site.occupancy") != std::string::npos){   
                        occup_n = i;
                    }
                    else if (line_string.find("_atom_site.B_iso_or_equiv") != std::string::npos){
                        bfac_n = i;
                    }
                }
                else{
